...
const userCtrl = {
  register: async (req, res) => {
    try {
      const { userName, email, password , description, walletConnected, walletAddress } = req.body;
      if(!walletConnected)return res.json({ error : "Wallet not connected"})
      if(email != walletAddress)return res.json({ error : "Entered wallet address does not match the connected wallet address"})
  ...
  login: async (req, res) => {
    try {
      const {email, password, walletConnected, walletAddress} = req.body;
      if(!walletConnected)return res.json({ error : "Wallet not connected"})
